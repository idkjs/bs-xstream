...
module Make_memory_stream = (Type: StreamTypes) => {
  ...
  [@bs.send] external addListener: (stream('a, 'b), listener) => unit = "addListener";
  [@bs.send] external removeListener: (stream('a, 'b), listener) => unit = "removeListener";
  [@bs.send]
  external subscribe: (stream('a, 'b), listener) => subscription = "subscribe";

  /* shame */
  [@bs.send] external shamefullySendNext: (stream('a, 'b), 'a) => unit = "shamefullySendNext";
  [@bs.send] external shamefullySendError: (stream('a, 'b), 'b) => unit = "shamefullySendError";
  [@bs.send]
  external shamefullySendComplete: (stream('a, 'b), unit) => unit = "shamefullySendComplete";
  [@bs.send]
  external setDebugListener: (stream('a, 'b), option(listener)) => unit = "setDebugListener";
};